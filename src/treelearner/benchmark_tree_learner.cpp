...
BenchmarkTreeLearner::BenchmarkTreeLearner(const Config *config)
    : config_(config)
{
...
BenchmarkTreeLearner::~BenchmarkTreeLearner()
{
#ifdef TIMETAG
...
void BenchmarkTreeLearner::Init(const Dataset *train_data, bool is_constant_hessian)
{
...
void BenchmarkTreeLearner::ResetTrainingData(const Dataset *train_data)
{
  train_data_ = train_data;
  num_data_ = train_data_->num_data();
...
void BenchmarkTreeLearner::ResetConfig(const Config *config)
{
...
Tree *BenchmarkTreeLearner::FitByExistingTree(const Tree *old_tree, const score_t *gradients, const score_t *hessians) const
{
...
Tree *BenchmarkTreeLearner::FitByExistingTree(const Tree *old_tree, const std::vector<int> &leaf_pred, const score_t *gradients, const score_t *hessians)
{
  data_partition_->ResetByLeafPred(leaf_pred, old_tree->num_leaves());
  return FitByExistingTree(old_tree, gradients, hessians);
}

std::vector<int8_t> BenchmarkTreeLearner::GetUsedFeatures(bool is_tree_level)
{
...
void BenchmarkTreeLearner::BeforeTrain()
{
...
bool BenchmarkTreeLearner::BeforeFindBestSplit(const Tree *tree, int left_leaf, int right_leaf)
{
  ...
  if (num_data_in_right_child < static_cast<data_size_t>(config_->min_data_in_leaf * 2) && num_data_in_left_child < static_cast<data_size_t>(config_->min_data_in_leaf * 2))
  {
    best_split_per_leaf_[left_leaf].gain = kMinScore;
    if (right_leaf >= 0)
    {
...
void BenchmarkTreeLearner::FindBestSplits()
{
...
void BenchmarkTreeLearner::ConstructHistograms(const std::vector<int8_t> &is_feature_used, bool use_subtract)
{
#ifdef TIMETAG
  auto start_time = std::chrono::steady_clock::now();
#endif
...
double BenchmarkTreeLearner::CalculateOndemandCosts(int feature_index, int leaf_index)
{
  if (config_->cegb_penalty_feature_lazy.empty())
    return 0.0f;
...
void BenchmarkTreeLearner::FindBestSplitsFromHistograms(const std::vector<int8_t> &is_feature_used, bool use_subtract)
{
#ifdef TIMETAG
  auto start_time = std::chrono::steady_clock::now();
#endif
...
int32_t BenchmarkTreeLearner::ForceSplits(Tree *tree, const Json &forced_split_json, int *left_leaf,
...
void BenchmarkTreeLearner::Split(Tree *tree, int best_leaf, int *left_leaf, int *right_leaf)
{
...
void BenchmarkTreeLearner::RenewTreeOutput(Tree *tree, const ObjectiveFunction *obj, std::function<double(const label_t *, int)> residual_getter,
