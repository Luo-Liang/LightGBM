...
void DataParallelTreeLearner<TREELEARNER_T>::InitializePHub()
{
  ...
  for (int i = 0; i < num_machines_; i++)
  {
    ...
    reduceScatterNodeFidOrder.back().reserve(numbin);
  }
...
void DataParallelTreeLearner<TREELEARNER_T>::BeforeTrain()
{
  ...
  for (int i = 0; i < num_machines_; ++i)
  {
    ...
    *(reduceScatterNodeByteCounters.at(i)) = 0;
    reduceScatterNodeFidOrder.at(i).clear();
...
void DataParallelTreeLearner<TREELEARNER_T>::FindBestSplits()
{
  ...
  for (int feature_index = 0; feature_index < this->num_features_; ++feature_index)
  {
    ...
    reduceScatterNodeFidOrder.at(nodeId).push_back(feature_index);
  ...
  {
    PHUB_CHECK(buffer_read_start_pos_.at(fid) + this->smaller_leaf_histogram_array_[fid].SizeOfHistgram() < pHubShadowCopy.size());
