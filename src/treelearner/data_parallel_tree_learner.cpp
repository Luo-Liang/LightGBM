...
namespace LightGBM
{
  ...
  void DataParallelTreeLearner<TREELEARNER_T>::InitializePHub()
  {
    ...
    size_t totalElementCount = this->train_data_->NumTotalBin();
    ...
    std::vector<int> keySizeInInt(keySizes.begin(), keySizes.end());

    getChunkedInformationGivenBuffer(pHubBackingBuffer.data(), totalElementCount, sizeof(HistogramBinEntry), chunkElements, keyCounts, keySizes, keyAddrs);

    var redisHost = pHubGetMandatoryEnvironmemtVariable("PHubRedisHost");
    var nodeMap = getPHubNodeMap(num_machines_, rank_, 0);
    pHub = std::make_shared<PHub>(redisHost, nodeMap, keySizeInInt, keyAddrs, num_machines_, sizeof(HistogramBinEntry), rank_, launchPref, PHubDataType::CUSTOM);
    pHub->SetReductionFunction(&PHubHistogramBinEntrySumReducer);

    PHubRendezvous scheduleRendezvous(redisIp, redisPort, rank, instanceId);
    scheduleRendezvous.Connect();
    if (rank_ == 0)
    {
      std::string keySizeStr;
      for (size_t i = 0; i < keyCounts.size(); i++)
      {
        keySizeStr += std::to_string(keyCounts.at(i)) + ",";
      }
      var scheduleFile = PHubGetUpdatedSchedule(0, keySizeStr);
      //	PHubRendezvous(string ip, uint port, NodeId myId, int instanceId, string pref = "PLINK") : IP(ip), Port(port), ID(myId), prefix(pref), gid(instanceId)
      std::ifstream i(scheduleFile);
      CHECK(i.good()) << " schedule file at " << scheduleFile << " is invalid.";
      std::string jstr;
      std::getline(i, jstr);
      scheduleRendezvous.PushKey("ScheduleJson", jstr);
      i.close();
    }
    ...
    //sleep(1);
    std::string scheduleJStr = scheduleRendezvous.waitForKey("ScheduleJson");
    auto pSchedule = std::make_shared<PHubSchedule>(scheduleJStr, rank, phubKeys, 0);
    pHub->InitializePHubThreading(pSchedule);
    scheduleRendezvous.Shutdown();
  }
