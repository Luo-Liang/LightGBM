...
void Network::AllreduceByAllGather(char *input, comm_size_t input_size, int type_size, char *output, const ReduceFunction &reducer)
{
  ...
  EASY_VALUE("AllreduceByAllGatherinput_size", input_size, EASY_VIN("AllreduceByAllGatherinput_size"));
...
void Network::Allgather(char *input, comm_size_t send_size, char *output)
{
  ...
  EASY_VALUE("Allgather3send_size", send_size, EASY_VIN("Allgather3send_size"));
  if (num_machines_ <= 1)
  {
    Log::Fatal("Please initilize the network interface first");
    return;
...
void Network::Allgather(char *input, const comm_size_t *block_start, const comm_size_t *block_len, char *output, comm_size_t all_size)
{
  ...
  EASY_ARRAY("Allgather5block_len", block_len, num_machines_, profiler::colors::Dark);
  if (num_machines_ <= 1)
  {
    Log::Fatal("Please initilize the network interface first");
  }
...
void Network::AllgatherBruck(char *input, const comm_size_t *block_start, const comm_size_t *block_len, char *output, comm_size_t all_size)
{
  ...
  EASY_ARRAY("AllgatherBruckblock_len", block_len, num_machines_, profiler::colors::Dark);
...
{
  EASY_FUNCTION(profiler::colors::Indigo100); 
  EASY_ARRAY("AllgatherRecursiveDoublingblock_len", block_len, num_machines_, profiler::colors::LightBlue100);
  // use output as receive buffer
...
{
  EASY_FUNCTION(profiler::colors::Indigo100); 
  EASY_ARRAY("AllgatherRingblock_len", block_len, num_machines_, profiler::colors::LightBlue100);
  // use output as receive buffer
  std::memcpy(output + block_start[rank_], input, block_len[rank_]);
                                            ...
                                            comm_size_t, const ReduceFunction &reducer)
{
  ...
  EASY_ARRAY("ReduceScatterRecursiveHalvingblock_len", block_len, num_machines_, profiler::colors::Navy);
                                ...
                                comm_size_t, const ReduceFunction &reducer)
{
  ...
  EASY_ARRAY("ReduceScatterRingblock_len", block_len, num_machines_, profiler::colors::Navy);
