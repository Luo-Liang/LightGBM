...
void GBDT::Train(int snapshot_freq, const std::string& model_output_path) {
  ...
  for (int iter = 0; iter < config_->num_iterations && !is_finished; ++iter) {
    ...
    Log::Info("[%d] %f seconds elapsed, finished iteration %d. exclusive network time = %f seconds (%f%%) Bytes on Wire:%fMB. SEND = %f seconds, and RECV = %f seconds. SENDRECV = %f", Network::rank(), speed, iter + 1, totalComm, 100.0 * totalComm / speed, bytesOnWire, Network::GetNetworkTime(NetworkTimeType::SEND), Network::GetNetworkTime(NetworkTimeType::RECV), Network::GetNetworkTime(NetworkTimeType::SENDRECV));
    if (snapshot_freq > 0
