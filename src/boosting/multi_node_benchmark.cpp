...
MultiNodeBenchmark::MultiNodeBenchmark() : iter_(0),
                                           train_data_(nullptr),
...
MultiNodeBenchmark::~MultiNodeBenchmark()
{
}

void MultiNodeBenchmark::Init(const Config *config, const Dataset *train_data, const ObjectiveFunction *objective_function,
...
void MultiNodeBenchmark::AddValidDataset(const Dataset *valid_data,
...
void MultiNodeBenchmark::Boosting()
{
    if (objective_function_ == nullptr)
    {
...
data_size_t MultiNodeBenchmark::BaggingHelper(Random *cur_rand, data_size_t start, data_size_t cnt, data_size_t *buffer)
{
    if (cnt <= 0)
    {
        return 0;
...
data_size_t MultiNodeBenchmark::BalancedBaggingHelper(Random *cur_rand, data_size_t start, data_size_t cnt, data_size_t *buffer)
{
    if (cnt <= 0)
    {
        return 0;
...
void MultiNodeBenchmark::Bagging(int iter)
{
    // if need bagging
    if ((bag_data_cnt_ < num_data_ && iter % config_->bagging_freq == 0) || need_re_bagging_)
    {
        need_re_bagging_ = false;
        const data_size_t min_inner_size = 1000;
        data_size_t inner_size = (num_data_ + num_threads_ - 1) / num_threads_;
        if (inner_size < min_inner_size)
        {
...
#pragma omp parallel for schedule(static, 1)
        for (int i = 0; i < num_threads_; ++i)
        {
            ...
            if (cur_start > num_data_)
            {
                continue;
            }
            data_size_t cur_cnt = inner_size;
            if (cur_start + cur_cnt > num_data_)
            {
                cur_cnt = num_data_ - cur_start;
            }
...
void MultiNodeBenchmark::Train(int snapshot_freq, const std::string &)
{
    ...
    for (int iter = 0; iter < config_->num_iterations; ++iter)
    {
        ...
        if (iter == config_->num_iterations - 1 || (snapshot_freq > 0 && (iter + 1) % snapshot_freq == 0))
        {
            ...
            fprintf(stderr, "benchmarked [%d:%s] avg iter = %fs. iter=%d. time = %s\n", Network::rank(), Network::GetHostName().c_str(), average, iter + 1, std::ctime(&end_time));
        }
    }
}

void MultiNodeBenchmark::RefitTree(const std::vector<std::vector<int>> &tree_leaf_prediction)
{
...
*
*/
//rename to silence linker.
double multiNodeBenchmark_ObtainAutomaticInitialScore(const ObjectiveFunction *fobj, int class_id)
{
...
double MultiNodeBenchmark::BoostFromAverage(int class_id, bool update_scorer)
{
    ...
    if (models_.empty() && !train_score_updater_->has_init_score() && objective_function_ != nullptr)
    {
        ...
        {
            double init_score = multiNodeBenchmark_ObtainAutomaticInitialScore(objective_function_, class_id);
        ...
        else if (std::string(objective_function_->GetName()) == std::string("regression_l1") || std::string(objective_function_->GetName()) == std::string("quantile") || std::string(objective_function_->GetName()) == std::string("mape"))
        {
...
void MultiNodeBenchmark::RollbackOneIter()
{
    if (iter_ <= 0)
    {
        return;
...
bool MultiNodeBenchmark::EvalAndCheckEarlyStopping()
{
...
void MultiNodeBenchmark::UpdateScore(const Tree *tree, const int cur_tree_id)
{
...
std::vector<double> MultiNodeBenchmark::EvalOneMetric(const Metric *metric, const double *score) const
{
    return metric->Eval(score, objective_function_);
}

std::string MultiNodeBenchmark::OutputMetric(int iter)
{
...
std::vector<double> MultiNodeBenchmark::GetEvalAt(int data_idx) const
{
...
const double *MultiNodeBenchmark::GetTrainingScore(int64_t *out_len)
{
    *out_len = static_cast<int64_t>(train_score_updater_->num_data()) * num_class_;
    return train_score_updater_->score();
}

void MultiNodeBenchmark::PredictContrib(const double *features, double *output, const PredictionEarlyStopInstance *early_stop) const
{
...
void MultiNodeBenchmark::GetPredictAt(int data_idx, double *out_result, int64_t *out_len)
{
    CHECK(data_idx >= 0 && data_idx <= static_cast<int>(valid_score_updater_.size()));
...
void MultiNodeBenchmark::ResetTrainingData(const Dataset *train_data, const ObjectiveFunction *objective_function,
                                           const std::vector<const Metric *> &training_metrics)
{
...
void MultiNodeBenchmark::ResetConfig(const Config *config)
{
...
void MultiNodeBenchmark::ResetBaggingConfig(const Config *config, bool is_change_dataset)
{
    // if need bagging, create buffer
    ...
    if ((config->bagging_fraction < 1.0 || balance_bagging_cond) && config->bagging_freq > 0)
    {
        ...
        if (!is_change_dataset &&
            config_.get() != nullptr && config_->bagging_fraction == config->bagging_fraction && config_->bagging_freq == config->bagging_freq && config_->pos_bagging_fraction == config->pos_bagging_fraction && config_->neg_bagging_fraction == config->neg_bagging_fraction)
        {
            return;
        }
        if (balance_bagging_cond)
        {
            ...
            bag_data_cnt_ = static_cast<data_size_t>(num_pos_data * config->pos_bagging_fraction) + static_cast<data_size_t>((num_data_ - num_pos_data) * config->neg_bagging_fraction);
        }
        else
        {
        ...
        if (average_bag_rate <= 0.5 && (train_data_->num_feature_groups() < group_threshold_usesubset || sparse_group < sparse_group_threshold_usesubset))
        {
